...
The workspace is split into focused crates under the top-level directory. `dsh/` hosts the shell binary (main entry point in `dsh/src/main.rs`) plus subsystems such as `completion/`, `process/`, and the `lisp/` interpreter. Supporting crates include `dsh-builtin/` for built-in commands, `dsh-frecency/` for directory ranking, `dsh-types/` for shared data structures, and `dsh-openai/` for AI integration. Contribution assets such as JSON completion definitions live in `completions/`, while shared formatting rules sit in `rustfmt.toml`.

## Build, Test, and Development Commands
Use `cargo build` for a fast debug build and `cargo build --release` when you need optimized binaries. Run the shell locally with `cargo run -p dsh --` followed by typical CLI flags (e.g. `cargo run -p dsh -- --help`). Execute the test suite with `cargo test` or scope to a crate, for example `cargo test -p dsh`. Before pushing, ensure formatting with `cargo fmt` and consider `cargo clippy --all-targets --all-features` for linting.

## Coding Style & Naming Conventions
Rust code follows the project `rustfmt.toml`, enforcing 4-space indentation and Edition 2024 defaults; run `cargo fmt` before committing. Favor descriptive snake_case for functions and modules, UpperCamelCase for types, and SCREAMING_SNAKE_CASE for constants. When adding modules, mirror the existing directory layout under `dsh/src/`—group related functionality (e.g. new completion providers under `completion/`).

## Testing Guidelines
Unit and integration tests live alongside source files (see `dsh/src/error_handling_tests.rs`). Prefer targeted unit tests inside the crate plus integration-style assertions using Tokio when async behavior is involved. Name tests after the scenario they validate, such as `handles_pipeline_abort`. Run `cargo test -- --nocapture` when diagnosing output-sensitive behaviors. Add regression tests for bugs, aiming to keep CI-friendly execution under a few seconds.
