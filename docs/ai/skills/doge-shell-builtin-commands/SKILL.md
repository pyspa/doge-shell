...
- Cron jobs (`cron`) live in `dsh-builtin/src/cron.rs` (thin wrapper) and `dsh/src/cron/` (store, clock, exec, CLI parsing); shared types are `dsh-types/src/cron.rs` and `dsh-types/src/schedule.rs`. `sched` is removed; the builtin name now prints a pointer to `cron`.
