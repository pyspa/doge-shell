...
- Default read targets are `dsh/src/history/`, `dsh/src/db.rs`, `dsh-frecency/src/`, `dsh/src/command_timing.rs`, `dsh-builtin/src/command_timing.rs`, `dsh-builtin/src/blocks.rs`, and `dsh-builtin/src/z.rs`.
...
- The optional `command_events` ledger is append-only and separate from aggregate history. Keep `(pref-command-ledger "off")` as the default; `off` and `metadata` must not inspect or store output, while `output` requires explicit opt-in, secret redaction, size and retention limits. Apply persistent filters before the result limit. Atuin integration must stay a non-blocking optional adapter.
