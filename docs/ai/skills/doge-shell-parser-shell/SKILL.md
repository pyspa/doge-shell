...
- Shell planning (`dsh/src/shell/plan.rs`, `dsh/src/shell/parse.rs`) is side-effect-free and performs no runtime word expansion. Execution planning preserves word structure (`PlannedWord` / `WordPart`); alias rewriting is syntax-time only (`parser::rewrite_aliases`, static `argv0` spans). Variable, tilde, brace/glob and substitution expansion happen only when a selected job is materialized (`dsh/src/shell/word_expand.rs`, via `dsh/src/shell/materialize.rs` and `dsh/src/shell/substitution.rs`). Every substitution body and the final materialized argv go through `SafetyGuard` via `dsh/src/shell/authorize.rs`.
