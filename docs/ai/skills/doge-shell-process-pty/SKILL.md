...
- Default read targets are `dsh/src/process/io.rs`, `dsh/src/process/job_pty.rs`, `dsh/src/process/pty.rs`, `dsh/src/shell/eval.rs`, and `dsh/src/terminal/`. Spawn-boundary work starts at `dsh/src/process/child_exec.rs` (raw post-fork child), `dsh/src/process/reexec/` (shared re-exec protocol), and `dsh/src/shell/process_substitution/` (direction-aware helper ownership).
...
- Foreground `JobLaunchOutcome` is derived from the canonical tree, never tail-only. Only a completed tree has a final exit status (`final_exit_status()` is `None` for incomplete trees under both pipefail policies).
...
- Stopped/incomplete jobs never write completed `OutputHistory`. Completion-only I/O finalization (`capture_completed_output_and_history`) never runs on stopped jobs.
- `fg` completion status comes from frozen `Job::final_exit_status()`. A job that stops again remains owned/requeued; `fg` reports the observed stop as 128+signal without turning that stop into a final `Job` status.
