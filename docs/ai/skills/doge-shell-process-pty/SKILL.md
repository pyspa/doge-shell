...
- Cron jobs do **not** go through this path: `dsh/src/cron/exec.rs` spawns a detached `sh -c` child with stdin on `/dev/null` and its own process group, with no PTY and no `Job`. `Shell` is `!Send`, so a spawned task cannot use `eval_str` — an agent job instead runs in its own `dogesh -c "cron run-job <uuid>"` child (`dsh/src/cron/run_job.rs`). See [../doge-shell-repo/references/invariants.md](../doge-shell-repo/references/invariants.md).
