...
| 変数 | 定義位置 | 用途 |
|---|---|---|
| `DSH_LOG` | `dsh/src/bootstrap.rs` | tracing の `EnvFilter`。**`RUST_LOG` ではない**。既定は `info`、出力はファイル |
...
| `DSH_HISTORY_PICKER` | `dsh/src/repl/init.rs` | `skim` を指定すると Ctrl-R が skim ピッカーになる |
| `DSH_COMPLETION_TIMING` | `dsh/src/completion/integrated/timing.rs` | 補完の所要時間を計測する |
...
| `DSH_EXTERNAL_COMPLETER` | `dsh/src/completion/dynamic.rs` / `dynamic/external.rs` | 外部コンプリータへ渡される（`DSH_COMPLETION_*` 一式と一緒に export される） |
...
| `AI_CHAT_PROJECT_SKILLS` | `dsh-builtin/src/chatgpt/settings.rs` | `0`/`false`/`off`/`no` で `<project>/.dsh/skills` をプロンプトから外す。既定 on |
