...
- MSRV 1.95 (`Cargo.toml` の `rust-version`)。`sysinfo 0.39.6` が 1.95 を要求するのが現在の床。旧根拠の `str::floor_char_boundary` (dsh-openai) は 1.91、`std::io::pipe` は 1.87。CI の `msrv (1.95)` job が `cargo check --workspace --all-targets` で実証する。
