...
- Every agent confirmation goes through `confirm_agent_action` in `dsh-builtin/src/chatgpt/tool/safety_gates.rs`, which offers Allow / AllowAlways / Deny. Do not add a `ShellProxy::confirm_action` call to a chat tool - the bool cannot say "always".
