...
- Start with `agent run [--tokens N] [--timeout SECONDS] [--check TEXT] [--write DIR]... -- GOAL` (budgets default to 50000 tokens / 900s). The `--` before the goal is required. `--check` is repeatable and each criterion is verified against a recorded tool result, so write it as something checkable, not a judgement call.
...
- Budgets come from flags, or from `AI_AGENT_TOKEN_BUDGET` / `AI_AGENT_TIMEOUT_SECS` (shell variable, then environment), else built-in defaults (50000 tokens / 900s). The token budget stops subsequent requests, not a billing cap. `AI_AGENT_MAX_CONCURRENT` (default 1) bounds how many tasks may run at once.
