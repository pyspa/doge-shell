...
| Time budget exhausted before the work is done | `--timeout` too small | `agent resume ID --timeout SECONDS` with a larger value |
...
| Task fails immediately on every run | Missing API key or unreachable provider | A scheduled job settles these as a `config` incident rather than retrying (see $dsh-cron); an interactive run reports the error directly - `agent doctor` covers neither, it reports stuck tasks and stale state |
