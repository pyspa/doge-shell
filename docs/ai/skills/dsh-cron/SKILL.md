...
- Add an AI job with `--agent`: `cron add --agent --name NAME [--timeout 10m] --write . --check '...' '<schedule>' -- '<goal>'`. Grants work exactly like `agent run`; nothing is asked at tick time, so a missing grant stalls the run instead of prompting.
...
- Time budget, grants and completion criteria for unattended AI jobs, plus the per-job notepad: [references/ai-jobs.md](references/ai-jobs.md).
