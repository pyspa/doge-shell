...
- Repository-specific procedures belong in `<project>/.dogesh/skills/`; portable ones in `~/.config/dogesh/skills/`. A project skill of the same name shadows a personal one.
- A bundled `scripts/` file always prompts before it runs, at every safety level. That is deliberate; do not design around it.
- The `!` chat runtime and an optional turn-end reviewer (`AI_CHAT_SKILL_REFLECT`, off by default) can propose a skill via `skill_manage` themselves. Depending on `AI_CHAT_SKILL_STAGING`, that write may be queued instead of applied immediately; a person reviews it with `skill pending` / `skill diff` / `skill approve` / `skill reject` before it reaches disk. This does not change how a skill should be authored - the same frontmatter and layout rules apply either way.
