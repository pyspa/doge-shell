        ...
        if let Some(headers) = request_headers {
            ...
            if let Some(if_modified_since) = headers.get(header::IF_MODIFIED_SINCE) {
                ...
                if let Ok(client_time_str) = client_time_res
                    ...
                    && let Ok(client_date) = httpdate::parse_http_date(client_time_str)
                {
                    // The client sends an RFC 1123 date. Return 304 when the
                    // resource has not been modified since it, i.e. when the
                    // file mtime truncated to whole seconds (the precision
                    // Last-Modified carries) is at or before that date.
                    let file_timestamp: u64 = file_time
                        .duration_since(SystemTime::UNIX_EPOCH)
                        .unwrap_or_default()
                        .as_secs();
                    let file_date =
                        SystemTime::UNIX_EPOCH + std::time::Duration::from_secs(file_timestamp);
...
pub async fn serve_file_handler(
    ...
    // Remove leading slash and decode percent-escapes so links generated by
    // the directory listing (which encodes spaces, quotes and non-ASCII as
    // %XX) resolve back to real file names.
    let relative_path = super::scanner::url_path_unescape(path.strip_prefix('/').unwrap_or(path));
    ...
    match fs::read_dir(dir_path) {
        Ok(entries) => {
            ...
            html.push_str(&format!(
                "<h1>🐕 Directory: {}</h1>",
                super::scanner::html_escape(&dir_path.display().to_string())
            ));
            html.push_str("<ul>");
            ...
            for entry in entries {
                ...
                let path = entry.path();
                let href = super::scanner::url_path_escape(&file_name_str);
                let label = super::scanner::html_escape(&file_name_str);

                if path.is_dir() {
                    html.push_str(&format!(
                        "<li><a href=\"{href}\" class=\"dir\">📁 {label}/</a></li>"
                    ));
                } else {
                    html.push_str(&format!(
                        "<li><a href=\"{href}\" class=\"file\">📄 {label}</a></li>"
                    ));
                }
            }
