...
fn test_generate_command_candidates() {
    ...
    let candidates = CommandGenerator::new(&db)
        .generate_command_candidates("gi")
        .unwrap();
...
fn subcommand_option_value_uses_value_type_script() {
    ...
    let parsed =
        CommandLineParser::new().parse("git restore --source ma", "git restore --source ma".len());
    let candidates = CompletionGenerator::new(&db)
        .generate_candidates(&parsed)
        .unwrap();
    let names: Vec<String> = candidates
...
fn subcommand_alias_uses_canonical_argument_scope() {
    ...
    let parsed = CommandLineParser::new().parse("pacman --sync pack", "pacman --sync pack".len());
    let candidates = CompletionGenerator::new(&db)
        .generate_candidates(&parsed)
        .unwrap();
    let names: Vec<String> = candidates
