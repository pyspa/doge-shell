...
mod tests {
    ...
    use crate::lisp::model::{Env, IntType, List, Symbol, Value};
    ...
    fn test_eval_basic_values() {
        ...
        // Test integers
        let result = eval(env.clone(), &Value::Int(IntType::from(42))).unwrap();
        assert_eq!(result, Value::Int(IntType::from(42)));
    ...
    fn test_symbol_lookup() {
        ...
        // Define a symbol
        env.borrow_mut()
            .define(Symbol::from("test-var"), Value::Int(IntType::from(123)));

        // Look it up
        let result = eval(env.clone(), &Value::Symbol(Symbol::from("test-var"))).unwrap();
        assert_eq!(result, Value::Int(IntType::from(123)));
    ...
    fn test_define_and_set() {
        ...
        assert_eq!(lookup, Value::Int(IntType::from(42)));

        // Test set
        let set_expr = Value::List(List::from(vec![
            Value::Symbol(Symbol::from("set")),
            Value::Symbol(Symbol::from("x")),
            Value::Int(IntType::from(100)),
        ]));

        let result = eval(env.clone(), &set_expr).unwrap();
        assert_eq!(result, Value::Int(IntType::from(100)));

        // Verify the symbol was updated
        let lookup = eval(env.clone(), &Value::Symbol(Symbol::from("x"))).unwrap();
        assert_eq!(lookup, Value::Int(IntType::from(100)));
    }
    ...
    fn test_lambda_creation_and_call() {
        ...
        assert_eq!(result, Value::Int(IntType::from(6)));
    }
    ...
    fn test_let_binding() {
        ...
        assert_eq!(result, Value::Int(IntType::from(30)));
    ...
    fn test_if_conditional() {
        ...
        assert_eq!(result, Value::Int(IntType::from(0)));
    ...
    fn test_function_definition_and_call() {
        ...
        let result = eval(env.clone(), &call_expr).unwrap();
        assert_eq!(result, Value::Int(IntType::from(25)));
    }
