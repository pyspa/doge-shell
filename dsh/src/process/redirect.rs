...
pub(crate) fn apply(redirects: &[Redirect], ctx: &mut Context) -> Result<AppliedRedirects> {
    // Check every target first: `apply` mutates `ctx` and opens files as it
    // goes, so bailing halfway would leave the context pointing at descriptors
    // that are closed on the way out.
    ...
    for redirect in redirects {
        ...
        saved.push((redirect.fd, current_slot(ctx, redirect.fd)));
        set_slot(ctx, redirect.fd, source);
        changed_stdin |= redirect.is_stdin();
    }
